...
- 2) Build the TMSL `model.bim` for a Direct Lake SemanticModel and submit it as a TWO-PART item definition (see g/h — `model.bim` + `definition.pbism`). Get these EXACTLY right — the Fabric items API rejects a malformed model.bim with HTTP 400 and the notebook then raises a generic 'Semantic model creation failed':
   ...
   g) CRITICAL — the SemanticModel item definition MUST contain TWO InlineBase64 parts. A model.bim-only definition is REJECTED by the LRO with 'Workload_FailedToParseFile: Required artifact is missing in definition.pbism' (this is the #1 reporting failure). Build BOTH parts:
      • pbism_part: path='definition.pbism', payload = base64(utf-8(json.dumps({""$schema"":""https://developer.microsoft.com/json-schemas/fabric/item/semanticModel/definitionProperties/1.0.0/schema.json"",""version"":""5.0"",""settings"":{""qnaEnabled"":false}}))). This core-settings artifact is MANDATORY for the TMSL format — never omit it.
      • bim_part: path='model.bim', payload = base64(utf-8(json.dumps(model_bim))) — the TMSL model from steps a–f.
   h) Assemble sm_body = {""displayName"": f'{target_lakehouse_name}_sm', ""type"": ""SemanticModel"", ""definition"": {""parts"": [pbism_part, bim_part]}} and create it via `semantic_model_id = _create_item(sm_body)` (the LRO-aware helper above). Do NOT write a bespoke status check that only allows 200/201 — that wrongly rejects the 202 async-accept response. The helper handles 200/201/202 and only raises on a real 4xx/5xx (surfacing status+body). Assert semantic_model_id is not None afterwards.
- 3) Create a Report bound to the new SemanticModel id. WRAP THE ENTIRE REPORT BUILD IN try/except: on ANY error, record a FAIL row, set report_id=None and CONTINUE — NEVER raise (the semantic model + its query test in step 5a are the real serving-layer gate; a missing report must not fail the whole layer). Like the semantic model, a Fabric Report (PBIR) item definition needs MULTIPLE InlineBase64 parts or it fails the SAME 'Required artifact is missing' way — at minimum: path='definition.pbir' = json.dumps({""version"":""4.0"",""datasetReference"":{""byConnection"":{""connectionString"":null,""pbiServiceModelId"":null,""pbiModelVirtualServerName"":""sobe_wowvirtualserver"",""pbiModelDatabaseName"":semantic_model_id,""name"":""EntityDataSource"",""connectionType"":""pbiServiceXmlaStyleLive""}}}) and path='report.json' = a minimal report layout (one page/section) referencing the fact measures. report_body = {""displayName"": f'{target_lakehouse_name}_rpt', ""type"": ""Report"", ""definition"": {""parts"": [pbir_part, reportjson_part]}}; report_id = _create_item(report_body) — all inside the try/except.
